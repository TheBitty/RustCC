use crate::parser::ast::{Program, Statement, Expression, BinaryOp, Type, UnaryOp};
...
impl Transform for VariableObfuscator {
            ...
            for statement in &function.body {
                if let Statement::VariableDeclaration { name, .. } = statement {
                    if !var_map.contains_key(name) {
                        ...
                        var_map.insert(name.clone(), format!("_${}_{}", rng.gen::<u32>() % 1000, new_name));
                    }
                }
            }
    ...
    fn name(&self) -> &'static str {
        "Advanced Variable Name Obfuscator"
    }
}
...
impl Transform for ControlFlowObfuscator {
    ...
    fn name(&self) -> &'static str {
        "Advanced Control Flow Obfuscator"
    }
}
...
impl ControlFlowObfuscator {
    ...
    fn flatten_control_flow(&self, function: &mut crate::parser::ast::Function, rng: &mut impl Rng) {
    ...
    fn insert_opaque_predicates(&self, function: &mut crate::parser::ast::Function, rng: &mut impl Rng) {
        ...
        if !function.body.is_empty() {
            ...
            let else_block = Statement::Block(vec![
                Statement::ExpressionStatement(Expression::IntegerLiteral(rng.gen_range(1..1000))),
            ]);
...
impl Transform for DeadCodeInserter {
    fn apply(&self, program: &mut Program) -> Result<(), String> {
        ...
        for function in &mut program.functions {
            ...
            let prefixes = ["counter", "index", "temp", "buffer", "size", "len", "offset", "ptr", "flag"];
    ...
    fn name(&self) -> &'static str {
        "Advanced Dead Code Inserter"
    }
}

impl DeadCodeInserter {
    // Helper method to add complex initialization code
    fn add_complex_initialization(&self, statements: &mut Vec<Statement>, dummy_vars: &[String], rng: &mut impl Rng) {
    ...
    // Insert more complex dead code
    fn insert_complex_dead_code(&self, statements: &mut Vec<Statement>, dummy_vars: &[String], rng: &mut impl Rng) {
        ...
        match rng.gen_range(0..5) {
            ...
            3 => {
                ...
                statements.push(Statement::ExpressionStatement(self.create_complex_expression(rng)));
            }
            _ => {
...
impl StringEncryptor {
    ...
    fn encrypt_strings_in_statements(&self, statements: &mut Vec<Statement>, rng: &mut impl Rng) {
        for statement in statements {
            match statement {
                ...
                Statement::If { condition, then_block, else_block } => {
                    *condition = self.encrypt_strings_in_expression(condition.clone(), rng);
                    ...
                    if let Statement::Block(stmts) = then_block.as_mut() {
                        ...
                        let new_then = self.encrypt_strings_in_statement(then_block.as_ref().clone(), rng);
                    ...
                    if let Some(else_stmt) = else_block {
                        if let Statement::Block(stmts) = else_stmt.as_mut() {
                            ...
                            let new_else = self.encrypt_strings_in_statement(else_stmt.as_ref().clone(), rng);
                ...
                Statement::For { condition, increment, body, .. } => {
                    if let Some(cond) = condition {
    ...
    fn encrypt_strings_in_expression(&self, expr: Expression, rng: &mut impl Rng) -> Expression {
        match expr {
            ...
            Expression::FunctionCall { name, mut arguments } => {
